...
!function (t, e) {
  ...
  t.prototype = {
    ...
    for (l = 0; l < n; l++) for (s = 0; s < e.length; s++) l < i[s].length && (C[p++] = i[s][l]);

    for (l = 0; l < o; l++) for (s = 0; s < e.length; s++) l < a[s].length && (C[p++] = a[s][l]);
  ...
  return m.prototype = {
    ...
    multiply: function (t) {
      for (var e = new Array(this.getLength() + t.getLength() - 1), r = 0; r < this.getLength(); r++) for (var n = 0; n < t.getLength(); n++) e[r + n] ^= c.gexp(c.glog(this.get(r)) + c.glog(t.get(n)));
    ...
    for (var n = r.length / 3, o = new Array(), i = 0; i < n; i++) for (var a = r[3 * i + 0], s = r[3 * i + 1], u = r[3 * i + 2], h = 0; h < a; h++) o.push(new v(s, u));
