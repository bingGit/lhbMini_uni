...
<view class="container">
  ...
  <view class="play-bttuon-view" :style="isHuawei ? 'height:120rpx' : ''">
    ...
    <navigator :hidden="!playerHidden" class="ablum-lock-view-text" v-if="bookid == 79 && authMsg.status == 0" target="miniProgram" open-type="navigate" app-id="wx4d08202062ac8130" path="/pages/service-card/service-card?id=100010" version="release">{{authMsg.text}}</navigator>
    ...
    <text v-else-if="authMsg.status == 1 && playerHidden && authMsg.expire_text.length != 0" :style="authMsg.expire_text.length == 0 ? '' : 'line-height:7rpx;'" @tap.stop="goCustom" class="ablum-lock-view-text">{{authMsg.expire_text}}</text>
  </view>
</view>
...
export default {
  ...
  methods: {
    ...
    preAudio: function () {
      var that = this;
      let sub_len = this.albums[getApp().globalData.g_audio_pid].audio.length;
