...
<view :class="'wux-class ' + classes.wrap" :style="style">
    <canvas :style="style" canvas-id="circle"></canvas>
...
baseComponent({
  ...
  methods: {
    ...
    draw(line = true) {
      ...
      const endAngle = counterclockwise ? p - (this.data.beginAngle + this.data.currentAngle) : this.data.beginAngle + this.data.currentAngle; // 创建 canvas 绘图上下文

      this.ctx = this.ctx || wx.createCanvasContext('circle', this); // 清除画布

      this.ctx.clearRect(0, 0, size, size); // 绘制背景
      ...
      if (line) {
        ...
        this.ctx.setLineWidth(strokeWidth); //this.ctx.setStrokeStyle(color)//设置纯色
        // 设置渐变颜色开始

        var g = this.ctx.createLinearGradient(0, 0, 180, 0); //创建渐变对象  渐变开始点和渐变结束点

        g.addColorStop(0, circleBg); //添加颜色点

        g.addColorStop(1, circleBg); //添加颜色点

        this.ctx.strokeStyle = g; //使用渐变对象作为圆环的颜色
        // 设置渐变颜色结束
